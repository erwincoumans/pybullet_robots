...
class Minitaur(object):
  ...
  def _RecordMassInfoFromURDF(self):
    ...
    self._leg_masses_urdf.append(
        pybullet.getDynamicsInfo(self.quadruped, LEG_LINK_ID[0])[
            0])
    self._leg_masses_urdf.append(
        pybullet.getDynamicsInfo(self.quadruped, MOTOR_LINK_ID[0])[
            0])
    ...
    if reload_urdf:
      ...
      if self._on_rack:
        pybullet.createConstraint(
            self.quadruped, -1, -1, -1, pybullet.JOINT_FIXED,
            [0, 0, 0], [0, 0, 0], [0, 0, 1])
    else:
      pybullet.resetBasePositionAndOrientation(
          self.quadruped, INIT_POSITION, INIT_ORIENTATION)
      pybullet.resetBaseVelocity(self.quadruped, [0, 0, 0],
                                              [0, 0, 0])
  ...
  def GetBasePosition(self):
    ...
    """
    position, _ = (
        pybullet.getBasePositionAndOrientation(self.quadruped))
    return position
  ...
  def GetMotorAngles(self):
    ...
    """
    motor_angles = [
        pybullet.getJointState(self.quadruped, motor_id)[0]
  ...
  def GetMotorVelocities(self):
    ...
    """
    motor_velocities = [
        pybullet.getJointState(self.quadruped, motor_id)[1]
        for motor_id in self._motor_id_list
    ]
  ...
  def GetMotorTorques(self):
    ...
    if self._accurate_motor_model_enabled or self._pd_control_enabled:
      ...
      motor_torques = [
          pybullet.getJointState(self.quadruped, motor_id)[3]
          for motor_id in self._motor_id_list
      ]
